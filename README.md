...
| [1] | Clock (+ Time-set) | HH:MM / SS:CC  (24시간제, Time-set 시 선택 자리 LED 점등) | [1] | R / N / C |
...
| SW[2] | 디스플레이 포맷 전환 |
| SW[3] | Time-set 모드 진입 (Clock 모드에서 활성화, 선택 자리 LED 점등) |
| SW[15] | Reset |

> PC 제어 모드 **활성화** 시 UART 입력을 통한 스위치/버튼 제어만 가능, **비활성화** 시 물리 입력만 가능
...
|------|---------|------|
| `M` | — | PC 제어 모드 토글 |
| `R` | BTN_U | Run / Stop (Stopwatch) / Up (Clock) |
| `N` | BTN_C | Time-set select (Clock) |
| `C` | BTN_D | Clear (Stopwatch) / Down (Clock) |
